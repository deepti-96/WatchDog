...
- Real: Rust detection engine, CUSUM metric shift detection, deploy correlation, log signature extraction, Supabase/SQLite/JSON incident persistence, Vercel serverless APIs, notes/status updates, exports, health endpoint, explanation caching, persisted triage-agent reports, and rollback decision briefs.
...
- Replaceable in production: JSONL and hosted demo triggers can be swapped for Prometheus/OpenTelemetry, deploy webhooks, CI/CD events, or queue-based ingestion while keeping the detection, storage, and triage workflow.
...
- Human-in-the-loop rollback approval connected to deployment tooling
